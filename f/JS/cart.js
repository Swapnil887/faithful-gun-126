...
let append = (data) => {
  ...
  data.forEach((el) => {
    ...
    img1.addEventListener("click", () => {
      alert(`${el.title} will be removed from list`);
      del_cart_Data(id);
    });
...
async function get_Cart_Data() {
  let res = await fetch("http://localhost:4500/userproduct/cart",
  {
...
async function del_cart_Data(id) {
  var x = await fetch(
    `http://localhost:4500/userproduct/cart/${id}`,
    {
      method: "DELETE",
